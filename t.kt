override suspend fun loadLinks(
    ...
    return try {
        ...
        if (!pxMr.isNullOrBlank()) {
            ...
            val sRegex = Regex("""var\s+_s\s*=\s*\[(.*?)\];""", RegexOption.DOT_MATCHES_ALL)
            val sMatches = sRegex.find(html)?.groupValues?.get(1)?.split(",")?.map { it.trim().replace("\"","") } ?: emptyList()

            pRegex.findAll(html).forEachIndexed { idx, pm ->
                val chunks = Regex("\"([^\"]+)\"").findAll(pm.groupValues[2]).map { it.groupValues[1] }.toList()
                val seqRaw = sMatches.getOrNull(idx) ?: ""
                val seq: List<Int> = try {
                    AppUtils.parseJson(seqRaw)
                } catch (e: Exception) { chunks.indices.toList() }

                val decryptedChunks = chunks.map { chunk ->
                    val hex = chunk.replace(Regex("[^0-9a-fA-F]"), "")
                    if (hex.isBlank()) return@map ""
                    val bytes = hex.chunked(2).map { it.toInt(16).toByte() }.toByteArray()
                    bytes.mapIndexed { i, b -> (b.toInt() xor secretBytes[i % secretBytes.size].toInt()).toByte() }.toByteArray()

package com.cinmana // تأكد من أن هذا يطابق اسم الحزمة الخاص بك (com.cinmana)
...
@file:Suppress("DEPRECATION") // لتجاهل تحذيرات الدوال القديمة

class CinmanaProvider : MainAPI() {
    override var name = "Cinmana"
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val details = app.get(detailsUrl).parsed<CinmanaItem>()
        ...
        if (details.videoType == 2) {
            ...
            val episodes = app.get(episodesUrl).parsedSafe<List<CinmanaEpisode>>()?.map {
                newEpisode(it) {
            ...
            return newTvSeriesLoadResponse(title, url, TvType.TvSeries, episodes) {
                ...
                this.contentRating = null // يتم إضافة contentRating
            }
        } else {
    ...
    override suspend fun loadLinks(
        ...
        app.get(videosUrl).parsedSafe<List<CinmanaVideo>>()?.forEach { video ->
            newExtractorLink(
                source = this.name,
                ...
                this.isM3u8 = video.videoUrl.contains(".m3u8")
            }.let { callback(it) }
        }

        app.get(subtitlesUrl).parsedSafe<CinmanaSubtitleHolder>()?.translations?.forEach { sub ->
    ...
    data class CinmanaItem(
        ...
        fun toSearchResponse(): SearchResponse {
            val validUrl = nb ?: return MovieSearchResponse("Error", "error", this@CinmanaProvider.name, TvType.Movie, null, null)
            return if (this.videoType == 2) {
                newTvSeriesSearchResponse(enTitle ?: "No Title", validUrl, TvType.TvSeries) {
                    this.posterUrl = imgObjUrl
                    this.year = this@CinmanaItem.year?.toIntOrNull()
                }
            } else {
    ...
    @Serializable
    data class CinmanaVideo(
        val videoUrl: String,
        val resolution: String?
    )

    @Serializable
    data class CinmanaSubtitleHolder(
        val translations: List<CinmanaSubtitleFile>?
    )

    @Serializable
    data class CinmanaSubtitleFile(
        val file: String,
        val name: String?
    )
}